...
def vref_ppoc_error_plot(tuning, figure_set: FigureSet, pkl_dir, output_dir, plot_rise_time=True):
    ...
    init_matplotlib()
    # print(f"---- {figure_set.scenarios}")
    if df_subset[f'Ppoc_AEMO_Settling_Time'].empty:
        print("df_subset[f'Ppoc_AEMO_Settling_Time'].empty")
        return
    
    max_sig_settling_time = max(max(df_subset[f'Ppoc_AEMO_Settling_Time']), 1)
    if pd.isna(max_sig_settling_time):
        max_sig_settling_time = 1
    ...
    # num_scenarios = len(df_subset)

    for i, scenario in df_subset.iterrows():
        ...
        if os.path.isfile(pkl_path):
            ...
            # print("\r"+"#" * int(60*(i/num_scenarios))+"-" * int(60-60*(i/num_scenarios)), end="")
    ...
    axs[1].set_xlabel('Time since Vref Disturbance (s)')
...
def vref_vpoc_plot(tuning, figure_set: FigureSet, pkl_dir, output_dir, plot_rise_time=True):
    df_subset = figure_set.scenarios
    fig_title = figure_set.title
    plot_filename = figure_set.filename
...
def vref_qpoc_error_plot(tuning, figure_set: FigureSet, pkl_dir, output_dir, plot_rise_time=False):
    df_subset = figure_set.scenarios
    fig_title = figure_set.title
    plot_filename = figure_set.filename
    ...
    for _, scenario in df_subset.iterrows():
        ...
        if os.path.isfile(pkl_path):
            ...
            pscad_signal = (data['POC_Q_MVAr'][plot_start:plot_end] - data['Qref_droop_MVAr'][plot_start:plot_end])
...
def plotting_function(project_tuning, analysis_df, data_root_dir, output_dirpath):
    ...
    # analysis_df = analysis_df.head(8)

    # print(f"analysis_df: {analysis_df.head(5)}")

    filename_prefix = "s52513_vpoc_steps_"
    ...
    filter = (analysis_df['Category'].isin(["d5%", "u5%", "Sat(d5%)", "Sat(u5%)"]))
    title = 'Vpoc Step: Ppoc Error\n(All Tests)'
    ...
    filter = (analysis_df['Category'].isin(["Sat(d5%)", "Sat(u5%)"]))
    title = f'Vpoc Step: Vpoc\n(5%-step Saturation)'
    filename = '5pc_Saturation_Vpoc.png'
    vpoc_settling_plots.append(FigureSet(analysis_df[filter], title, filename_prefix + filename))

    for fset in vpoc_settling_plots:
        print(f"Saving {fset.title}")
        vref_vpoc_plot(project_tuning, fset, data_root_dir, output_dirpath, plot_rise_time=False)


    # --- Qpoc Plots
    qpoc_settling_plots = []
    filter = (analysis_df['Category'].isin(["d5%", "u5%", "Sat(d5%)", "Sat(u5%)"]))
    title = 'Vpoc Step: Qpoc Error\n(All Tests)'
