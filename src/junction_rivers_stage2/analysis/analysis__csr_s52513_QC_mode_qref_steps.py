...
def per_scenario_analysis(tuning: dict, scenario: dict, run_data: pd.DataFrame):
    ...
    qref_steps = json.loads(scenario['Qref_Deltas'])
    ...
    qref_step_start = time_steps[0] + model_init_time
    qref_step_end = time_steps[1] + model_init_time

    # This is added in to avoid seeing OLTC taps. Remove this if the analysis does not converge.
    if qref_step_end - qref_step_start > 7:
        qref_step_end = qref_step_start + 7
    ...
    pre_ppoc_settled, pre_ppoc_mw, _ = signal_analysis.calc_settled_value(ppoc_mw, qref_step_start, min_settle_tol,
                                                                          ppoc_tol_mw)
    post_ppoc_settled, post_ppoc_mw, _ = signal_analysis.calc_settled_value(ppoc_mw, qref_step_end, min_settle_tol,
                                                                            ppoc_tol_mw)

    pre_qpoc_settled, pre_qpoc_mvar, _ = signal_analysis.calc_settled_value(qpoc_mvar, qref_step_start, min_settle_tol,
                                                                            qpoc_tol_mvar)
    post_qpoc_settled, post_qpoc_mvar, _ = signal_analysis.calc_settled_value(qpoc_mvar, qref_step_end, min_settle_tol,
                                                                              qpoc_tol_mvar)

    pre_vpoc_settled, pre_vpoc_pu, _ = signal_analysis.calc_settled_value(vpoc_pu, qref_step_start, min_settle_tol,
                                                                          vpoc_pu_tol)
    post_vpoc_settled, post_vpoc_pu, post_vpoc_settled_time = signal_analysis.calc_settled_value(vpoc_pu, qref_step_end, min_settle_tol,
                                                                            vpoc_pu_tol)

    pre_qref_settled, pre_qref_mvar, _ = signal_analysis.calc_settled_value(qref_mvar, qref_step_start, min_settle_tol,
                                                                            4*qpoc_tol_mvar)
    post_qref_settled, post_qref_mvar, _ = signal_analysis.calc_settled_value(qref_mvar, qref_step_end, min_settle_tol,
                                                                              4*qpoc_tol_mvar)
    ...
    if analysis_map[f"Ppoc_Settled"]:
        p_error_sig = ppoc_mw[qref_step_start:qref_step_end] - pre_ppoc_mw
        ...
        t_settle = signal_analysis.calc_aemo_step_settling_time(ppoc_mw, qref_step_start, ppoc_disturbance_mw, qref_step_end,
                                                                post_ppoc_mw)
    ...
    if analysis_map[f"Qpoc_Settled"]:
        ...
        t_rise = signal_analysis.calc_aemo_step_rise_time(qpoc_mvar, qref_step_start, pre_qpoc_mvar, qref_step_end, post_qpoc_mvar)
        t_settle = signal_analysis.calc_aemo_step_settling_time(qpoc_mvar, qref_step_start, pre_qpoc_mvar, qref_step_end, post_qpoc_mvar)
    ...
    if analysis_map[f"Vpoc_Settled"]:
        ...
        t_settle = signal_analysis.calc_aemo_step_settling_time(vpoc_pu, qref_step_start, pre_vpoc_pu, qref_step_end, post_vpoc_pu)
        _, _, _, _, vpoc_ht = signal_analysis.calc_transient_characteristics(vpoc_pu, qref_step_start, pre_vpoc_pu, qref_step_end, post_vpoc_pu)
    ...
    wt1_v_pu = data["WT1_V_pu"][qref_step_start:qref_step_end]
    wt2_v_pu = data["WT2_V_pu"][qref_step_start:qref_step_end]
    wt3_v_pu = data["WT3_V_pu"][qref_step_start:qref_step_end]
    wt4_v_pu = data["WT4_V_pu"][qref_step_start:qref_step_end]
...
if __name__ == '__main__':
    analysis_df = standard_analysis_main(
        file_name_prefix=["s52513_QC_QrefSteps"], per_scenario_analysis=per_scenario_analysis,
