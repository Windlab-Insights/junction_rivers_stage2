...
def per_scenario_analysis(tuning: dict, scenario: dict, run_data: pd.DataFrame):
    ...
    vref_steps = json.loads(scenario['Vref_Deltas'])
    ...
    if vref_step_end - vref_step_start > 7:
        vref_step_end = vref_step_start + 7
    ...
    if analysis_map[f"Ppoc_Settled"]:
        p_error_sig = ppoc_mw[vref_step_start:vref_step_end] - pre_ppoc_mw
        ...
        t_settle = signal_analysis.calc_aemo_step_settling_time(ppoc_mw, vref_step_start, ppoc_disturbance_mw, vref_step_end,
                                                                post_ppoc_mw)
...
if __name__ == '__main__':
    analysis_df = standard_analysis_main(
        file_name_prefix=["s52513_VrefSteps"], per_scenario_analysis=per_scenario_analysis,
