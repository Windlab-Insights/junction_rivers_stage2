...
def qref_ppoc_error_plot(tuning, figure_set: FigureSet, pkl_dir, output_dir, plot_rise_time=True):
    ...
    #---------------------------
    print("investig")
    # k = df_subset['Ppoc_AEMO_Settling_Time']
    # print(f'PPOC_ST: {k}')
    ...
    axs[1].set_xlabel('Time since Qref Disturbance (s)')
...
def qref_vpoc_plot(tuning, figure_set: FigureSet, pkl_dir, output_dir, plot_rise_time=True):
    ...
    print("investig")
    k = df_subset['Vpoc_AEMO_Settling_Time']
    print(f'VPOC_ST: {k}') 
    print(df_subset.head())
    
    # if df_subset['Vpoc_AEMO_Settling_Time'].empty:
    #     print("success")
    #     return
...
def qref_qpoc_error_plot(tuning, figure_set: FigureSet, pkl_dir, output_dir):
    df_subset = figure_set.scenarios
    fig_title = figure_set.title
    plot_filename = figure_set.filename
    ...
    for _, scenario in df_subset.iterrows():
        ...
        if os.path.isfile(pkl_path):
            ...
            pscad_signal = (data['POC_Q_MVAr'][plot_start:plot_end] - data['Qref_MVAr'][plot_start:plot_end])
...
def plotting_function(project_tuning, analysis_df, data_root_dir, output_dirpath):
    ...
    filename_prefix = "s52513_Qref_steps_"
    ...
    filter = (analysis_df['Category'].isin(["d50%", "u50%"]))
    title = f'Q Control Mode Qref Step: Ppoc Error\n(All Tests)'
    filename = 'Q_ControlMode_All_Ppoc_Error.png'
    ...
    # filter = (analysis_df['Category'].isin(["d50%", "u50%"]))
    # title = f'Q Control Mode Qref Step: Ppoc Error\n(50%-step No Saturation)'
    # filename = 'Q_ControlMode_50pc_NoSaturation_Ppoc_Error.png'
    # ppoc_settling_plots.append(FigureSet(analysis_df[filter], title, filename_prefix + filename))
    ...
    # filter = (analysis_df['Category'].isin(["d50%", "u50%"]))
    # title = f'Q Control Mode Qref Step: Ppoc Error\n(50%-step Saturation)'
    # filename = 'Q_ControlMode_50pc_Saturation_Ppoc_Error.png'
    # ppoc_settling_plots.append(FigureSet(analysis_df[filter], title, filename_prefix + filename))
    ...
    # filter = (~analysis_df['Category'].str.contains(exclude_categories))
    filter = (analysis_df['Category'].isin(["d50%", "u50%"]))
    title = 'Q Control Mode Qref Step: Vpoc\n(All Tests)'
    filename = 'Q_ControlMode_All_Vpoc.png'
    vpoc_settling_plots.append(FigureSet(analysis_df[filter], title, filename_prefix + filename))

    # # filter = (analysis_df['Category'].str.contains(include_categories))
    # filter = (analysis_df['Category'].isin(["d50%", "u50%"]))
    # title = f'Q Control Mode Qref Step: Vpoc\n(50%-step No Saturation)'
    # filename = 'Q_ControlMode_50pc_No_Saturation_Vpoc.png'
    # vpoc_settling_plots.append(FigureSet(analysis_df[filter], title, filename_prefix + filename))

    # include_categories = "|".join([re.escape(x) for x in ["Sat(d50%)", "Sat(u50%)"]])
    # filter = (analysis_df['Category'].isin(["Sat(d50%)", "Sat(u50%)"]))
    # title = f'Q Control Mode Qref Step: Vpoc\n(50%-step Saturation)'
    # filename = 'Q_ControlMode_50pc_Saturation_Vpoc.png'
    # vpoc_settling_plots.append(FigureSet(analysis_df[filter], title, filename_prefix + filename))

    for fset in vpoc_settling_plots:
        print(f"Saving {fset.title}")
        qref_vpoc_plot(project_tuning, fset, data_root_dir, output_dirpath)


    # --- Qpoc Plots
    qpoc_settling_plots = []
    filter = (analysis_df['Category'].isin(["d50%", "u50%"]))
    title = 'Q Control Mode Qref Step: Qpoc Error\n(All Tests)'
    filename = 'Q_ControlMode_All_Qpoc_Error.png'
    ...
    # # include_categories = "|".join([re.escape(x) for x in ["Sat(d5%)", "Sat(u5%)"]])
    # include_categories = ["d50%", "u50%"]
    # filter = (analysis_df['Category'].isin(include_categories))
    # title = f'Q Control Mode Qref Step: Qpoc Error\n(50%-step No Saturation)'
    # filename = 'Q_ControlMode_50pc_No_Saturation_Qpoc_Error.png'
    # qpoc_settling_plots.append(FigureSet(analysis_df[filter], title, filename_prefix + filename))


    # # include_categories = "|".join([re.escape(x) for x in ["PLQSat(d5%)", "PLQSat(u5%)"]])
    # include_categories = ["Sat(d50%)", "Sat(u50%)"]
    # filter = (analysis_df['Category'].isin(include_categories))
    # title = f'Q Control Mode Qref Step: Qpoc Error\n(50% Step Saturation)'
    # filename = 'Q_ControlMode_50pc_Saturation_Qpoc_Error.png'
    # qpoc_settling_plots.append(FigureSet(analysis_df[filter], title, filename_prefix + filename))
